...
//months
char M_arr[12][5] = { { '.', 'J', 'a', 'n', '.' }, { '.', 'F', 'e', 'b', '.' }, { '.', 'M', 'a', 'r', '.' },
                      { '.', 'A', 'p', 'r', '.' }, { '.', 'M', 'a', 'y', ' ' }, { '.', 'J', 'u', 'n', 'e' },
                      { '.', 'J', 'u', 'l', 'y' }, { '.', 'A', 'u', 'g', '.' }, { '.', 'S', 'e', 'p', 't' },
                      { '.', 'O', 'c', 't', '.' }, { '.', 'N', 'o', 'v', '.' }, { '.', 'D', 'e', 'c', '.' } };
//days
char WD_arr[7][4] = { { 'S', 'u', 'n', ',' }, { 'M', 'o', 'n', ',' }, { 'T', 'u', 'e', ',' }, {'W', 'e', 'd', ',' },
                      { 'T', 'h', 'u', ',' }, { 'F', 'r', 'i', ',' }, { 'S', 'a', 't', ',' } };

// Font 5x8 for 8x8 matrix, 0,0 is above right
const uint8_t font1[96][9] = {
        ...
        if (_f_tckr50ms == true) {
            ...
            if (sc1 == 1) {
                if (updown == 1)
                    ...
                    y++;
                char2Arr(48 + sek12, _zPosX - 42, y);
                char2Arr(48 + sek11, _zPosX - 42, y + y1);
                if (y == 0) {
                    sc1 = 0;
                    f_scrollend_y = true;
                }
            }
            else char2Arr(48 + sek1, _zPosX - 42, 0);

            if (sc2 == 1) {
                char2Arr(48 + sek22, _zPosX - 36, y);
                char2Arr(48 + sek21, _zPosX - 36, y + y1);
                if (y == 0)
                    sc2 = 0;
            }
            else char2Arr(48 + sek2, _zPosX - 36, 0);

            char2Arr(':', _zPosX - 32, 0);

            if (sc3 == 1) {
                char2Arr(48 + min12, _zPosX - 25, y);
                char2Arr(48 + min11, _zPosX - 25, y + y1);
                if (y == 0)
                    sc3 = 0;
            }
            else char2Arr(48 + min1, _zPosX - 25, 0);

            if (sc4 == 1) {
                char2Arr(48 + min22, _zPosX - 19, y);
                char2Arr(48 + min21, _zPosX - 19, y + y1);
                if (y == 0)
                    sc4 = 0;
            }
            else char2Arr(48 + min2, _zPosX - 19, 0);

            char2Arr(':', _zPosX - 15 + x, 0);

            if (sc5 == 1) {
                char2Arr(48 + std12, _zPosX - 8, y);
                char2Arr(48 + std11, _zPosX - 8, y + y1);
                if (y == 0)
                    sc5 = 0;
            }
            else char2Arr(48 + std1, _zPosX - 8, 0);

            if (sc6 == 1) {
                char2Arr(48 + std22, _zPosX - 2, y);
                char2Arr(48 + std21, _zPosX - 2, y + y1);
                if (y == 0)
                    sc6 = 0;
            }
            else char2Arr(48 + std2, _zPosX - 2, 0);

            char2Arr(WD_arr[rtc.getweekday()][0], _dPosX - 5, 0);    //day of the week
            char2Arr(WD_arr[rtc.getweekday()][1], _dPosX - 11, 0);
            char2Arr(WD_arr[rtc.getweekday()][2], _dPosX - 17, 0);
            char2Arr(WD_arr[rtc.getweekday()][3], _dPosX - 23, 0);
            char2Arr(48 + rtc.getday()/10, _dPosX - 27, 0);          //day
            char2Arr(48 + rtc.getday()%10, _dPosX - 33, 0);
            char2Arr(M_arr[rtc.getmonth() ][0], _dPosX - 39, 0);     //month
            char2Arr(M_arr[rtc.getmonth() ][1], _dPosX - 43, 0);
            char2Arr(M_arr[rtc.getmonth() ][2], _dPosX - 49, 0);
            char2Arr(M_arr[rtc.getmonth() ][3], _dPosX - 55, 0);
            char2Arr(M_arr[rtc.getmonth() ][4], _dPosX - 61, 0);
            char2Arr('2', _dPosX - 68, 0);                           //year
            char2Arr('0', _dPosX - 74, 0);
            char2Arr(48 + rtc.getyear()/10, _dPosX - 80, 0);
            char2Arr(48 + rtc.getyear()%10, _dPosX - 86, 0);
